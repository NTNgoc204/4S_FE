function ChatConversationPanel({
  ...
  const availableQuickPrompts = quickPrompts
    .map((prompt, index) => ({ prompt, index }))
    .filter((item) => !usedPromptIndexes.includes(item.index))
  ...
  return (
    <div className="flex min-h-0 flex-col border-r border-white/10">
        ...
        {messages.map((message) =>
          message.role === 'assistant' ? (
            <div key={message.id} className="flex items-start gap-3">
              ...
              <p className="max-w-[780px] rounded-2xl border border-[#5f7396]/45 bg-gradient-to-b from-[#213a58]/95 to-[#182f4a]/98 px-4 py-3 text-[1rem] leading-8 text-slate-100 md:px-5">
                {message.content}
              </p>
            </div>
          ) : (
      ...
      {availableQuickPrompts.length > 0 ? (
        <div className="border-t border-white/10 bg-[#102742]/85 px-4 py-3 md:px-6">
          <div className="flex gap-2 overflow-x-auto whitespace-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {availableQuickPrompts.map((item) => (
              <button
                key={`${item.index}-${item.prompt}`}
                className="rounded-full border border-white/12 bg-white/[0.05] px-4 py-2 text-sm text-slate-300 transition hover:border-[#ecc741]/40 hover:text-slate-100"
                onClick={() => onQuickPrompt(item.index)}
                type="button"
              >
                {item.prompt}
              </button>
