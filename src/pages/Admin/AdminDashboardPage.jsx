    ...
    <section className="space-y-6">
      <header className="rounded-2xl border border-white/10 bg-[#153251]/82 p-5 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="font-['Sora'] text-2xl font-semibold md:text-3xl">Admin Dashboard</h2>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className="rounded-xl border border-[#0ed8ab]/35 bg-[#0ed8ab]/14 px-4 py-2 text-sm font-semibold text-[#0ed8ab] transition hover:bg-[#0ed8ab]/24"
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {KPI_CARDS.map((item) => (
          <article key={item.id} className="rounded-2xl border border-white/10 bg-[#183452]/82 p-4 md:p-5">
            <p className="text-sm text-slate-300">{item.label}</p>
            <p className="mt-2 font-['Sora'] text-3xl font-semibold">{item.value}</p>
            <span className="mt-3 inline-flex rounded-full border border-[#0ed8ab]/30 bg-[#0ed8ab]/12 px-2.5 py-1 text-xs font-semibold text-[#0ed8ab]">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
        <article className="rounded-2xl border border-white/10 bg-[#183452]/82 p-5">
          <h3 className="font-['Sora'] text-xl font-semibold">Traffic & Acquisition</h3>
          <p className="mt-1 text-sm text-slate-300">Where users come from this month</p>
            ...
            {CHANNEL_PERFORMANCE.map((channel) => (
              <div key={channel.name}>
                <div className="mb-2 flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-[#0ed8ab]">{channel.value}</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-white/12">
                  <span
                    className="block h-full rounded-full bg-gradient-to-r from-[#19d2ad] to-[#0fbf9b]"
        ...
        <article className="rounded-2xl border border-white/10 bg-[#183452]/82 p-5">
          <h3 className="font-['Sora'] text-xl font-semibold">Plan Distribution</h3>
          <p className="mt-1 text-sm text-slate-300">Users grouped by current package</p>

          <div className="mt-5 space-y-3">
            {PLAN_BREAKDOWN.map((item) => (
              <div key={item.name} className="rounded-xl border border-white/10 bg-[#10253e]/75 p-3">
                <div className="mb-2 flex items-center justify-between text-sm">
                  <p className="font-semibold text-slate-100">{item.name}</p>
                  <p className="text-slate-300">{item.users.toLocaleString()} users</p>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-white/12">
                  <span
                ...
                <p className="mt-1 text-xs text-slate-400">{item.ratio}% of active base</p>
              </div>
            ))}
          </div>
        </article>
      ...
      <article className="rounded-2xl border border-white/10 bg-[#183452]/82 p-5">
        <div className="flex items-center justify-between gap-3">
          <h3 className="font-['Sora'] text-xl font-semibold">Recent Admin Activity</h3>
          <button
            className="rounded-lg border border-white/12 bg-white/6 px-3 py-1.5 text-sm text-slate-200 transition hover:bg-white/12"
            type="button"
          >
            Export CSV
          </button>
          ...
          <table className="min-w-full border-separate border-spacing-y-2">
            ...
            <tbody>
              {RECENT_ACTIVITY.map((row) => (
                <tr key={row.id} className="rounded-xl border border-white/10 bg-[#10253e]/75">
                  <td className="rounded-l-xl px-3 py-3 text-sm text-slate-100">{row.action}</td>
                  <td className="px-3 py-3 text-sm font-semibold text-[#0ed8ab]">{row.amount}</td>
                  <td className="rounded-r-xl px-3 py-3 text-sm text-slate-300">{row.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
