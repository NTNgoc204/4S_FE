  ...
  return (
    <>
          ...
          <table className="min-w-full">
            <thead className="border-b border-slate-200 text-left text-xs uppercase tracking-[0.12em] text-slate-500 bg-slate-50 font-semibold">
              <tr>
                ...
                <th className="px-4 py-3.5">{isVi ? "Mã kích hoạt (Key)" : "Activation Key"}</th>
                ...
                registrations.map((item) => (
                  <tr className="hover:bg-slate-50/50 transition-colors" key={item.id}>
                    ...
                    <td className="px-4 py-4.5 whitespace-nowrap">
                      ...
                      >
                        {item.status === "Pending" && (isVi ? "Chờ tiếp nhận" : "Pending")}
                        {item.status === "Quoted" && (isVi ? "Đã báo giá" : "Quote Issued")}
                        {item.status === "Paid" && (isVi ? "Đã thanh toán" : "Paid")}
                        {item.status === "KeyGenerated" && (isVi ? "Đã cấp key" : "Key Created")}
                      </span>
                    </td>
                    <td className="px-4 py-4.5 whitespace-nowrap">
                      {item.activationKey ? (
                        <code className="rounded bg-slate-100 border border-slate-200 px-2 py-1 text-xs font-mono font-bold text-slate-800 tracking-wide select-all">
                          {item.activationKey}
                        </code>
                      ) : (
                    ...
                    <td className="px-4 py-4.5 whitespace-nowrap text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        {item.status === "Paid" && (
                          <button
                            className="rounded-xl bg-emerald-600 hover:bg-emerald-700 px-4 py-1.5 text-xs font-bold text-white transition shadow-sm cursor-pointer"
                            onClick={() => onGenerateKey(item)}
                            type="button"
                          >
                            {isVi ? "Sinh Key" : "Generate Key"}
                          </button>
                        )}
                        {item.status === "KeyGenerated" && (
                          <span className="text-xs text-emerald-600 font-bold border border-emerald-150 bg-emerald-50/50 rounded-lg px-2.5 py-1 select-all">
                            {isVi ? "Đã kích hoạt" : "Activated"}
                          </span>
                        )}
                      </div>
                    </td>
      ...
      <section className="space-y-3.5 md:hidden">
        {registrations.length === 0 ? (
          <p className="text-center py-8 text-sm text-slate-400">{isVi ? "Không tìm thấy đơn đăng ký nào." : "No school registrations found."}</p>
        ) : (
          registrations.map((item) => (
            <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3" key={item.id}>
              ...
              {item.activationKey && (
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 flex flex-col gap-1">
                  <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">{isVi ? "Mã kích hoạt (Key)" : "Activation Key"}</span>
                  <code className="text-xs font-mono font-bold text-slate-800 tracking-wide select-all">{item.activationKey}</code>
                </div>
              )}
              ...
              <div className="flex justify-between items-center pt-2.5 border-t border-slate-100">
                ...
                <div className="flex gap-2">
                  ...
                  {item.status === "Paid" && (
                    <button
                      className="rounded-xl bg-emerald-600 hover:bg-emerald-700 px-4 py-1.5 text-xs font-bold text-white transition cursor-pointer"
                      onClick={() => onGenerateKey(item)}
                      type="button"
                    >
                      {isVi ? "Sinh Key" : "Generate Key"}
